...
const INITIAL_TICKETS = [
  { id: "TKT-001", userId: "u1", userName: "Thabo Nkosi", department: "IT", description: "My laptop screen flickering and won't connect to the office Wi-Fi network.", status: "Resolved", date: "2025-05-20" },
  { id: "TKT-002", userId: "u1", userName: "Thabo Nkosi", department: "HR", description: "I need to apply for annual leave for the last week of June.", status: "Pending", date: "2025-05-22" },
  { id: "TKT-003", userId: "u2", userName: "Amara Dlamini", department: "Finance", description: "My expense claim from April has not been reimbursed yet.", status: "Open", date: "2025-05-24" },
  { id: "TKT-004", userId: "u2", userName: "Amara Dlamini", department: "Operations", description: "The air conditioning in meeting room B3 is broken.", status: "Pending", date: "2025-05-25" },
  { id: "TKT-005", userId: "u3", userName: "Sipho Mthembu", department: "IT", description: "Cannot access the shared drive. Getting a permissions error.", status: "Open", date: "2025-05-26" },
  { id: "TKT-006", userId: "u3", userName: "Sipho Mthembu", department: "Finance", description: "Need reimbursement for business travel to Cape Town last month.", status: "Resolved", date: "2025-05-27" },
  { id: "TKT-007", userId: "u1", userName: "Thabo Nkosi", department: "Operations", description: "Parking bay 14 is being used by an unauthorised vehicle daily.", status: "Open", date: "2025-05-28" },
  { id: "TKT-008", userId: "u2", userName: "Amara Dlamini", department: "IT", description: "Outlook keeps crashing every time I open a large attachment.", status: "Pending", date: "2025-05-29" },
  { id: "TKT-009", userId: "u3", userName: "Sipho Mthembu", department: "HR", description: "Need a letter confirming my employment for a bank loan application.", status: "Resolved", date: "2025-05-30" },
  { id: "TKT-010", userId: "u1", userName: "Thabo Nkosi", department: "Finance", description: "My March salary was short by R800. Please investigate.", status: "Open", date: "2025-05-31" },
];
...
const LIGHT = {
  ...
  inputBg: "#F8FAFC", headerBg: "#FFFFFF",
};
const DARK = {
...
function StatusBadge({ status, dark }) {
  ...
  const color = dark ? c.darkText : c.text;
  return (
    <span style={{ background: bg, color, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 5 }}>
...
function Modal({ show, onClose, title, children, type = "info", btnLabel = "Got it", dark }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ background: t.surface, borderRadius: 20, padding: "2rem", maxWidth: 420, width: "100%", boxShadow: "0 24px 80px rgba(0,0,0,0.4)", animation: "popIn 0.2s ease", border: `1px solid ${t.border}` }}>
...
function ThemeToggle({ dark, setDark }) {
  return (
    <button onClick={() => setDark(d => !d)} style={{ background: "none", border: "1px solid " + (dark ? "#334155" : "#E2E8F0"), borderRadius: 10, padding: "6px 12px", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", gap: 6, color: dark ? "#94A3B8" : "#64748B", fontFamily: "inherit" }} title="Toggle theme">
      {dark ? "☀️" : "🌙"}
    </button>
  );
}
...
function LandingPage({ onNav }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "#060C1A", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
      ...
      {/* Nav — logo + auth buttons only, no badge */}
      ...
      {/* Hero — no "Powered by Claude AI" badge */}
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "8rem 2rem 5rem", textAlign: "center", animation: "fadeUp 0.7s ease" }}>
        ...
        <p style={{ fontSize: 18, color: "#94A3B8", lineHeight: 1.75, maxWidth: 540, margin: "0 auto 3rem" }}>
          Describe your issue in plain English. OpsBridge intelligently classifies and routes it to the right department — instantly, no forms needed.
        </p>
      ...
      {/* Features */}
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 2rem 8rem" }}>
        <div style={{ textAlign: "center", marginBottom: "3.5rem" }}>
          <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 34, fontWeight: 800, letterSpacing: -1.5, margin: "0 0 0.5rem" }}>Everything your team needs</h2>
          <p style={{ color: "#475569", fontSize: 16 }}>Three roles, one seamless bridge</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 16 }}>
          {[
            { icon: "🤖", title: "AI Classification", desc: "Reads free-text requests and silently routes them — no category dropdowns ever needed." },
            { icon: "📊", title: "Admin Dashboard", desc: "Per-department admins manage their queue with stats, filters, and live status updates." },
            { icon: "👑", title: "Super Admin View", desc: "One command-centre showing every department, every metric, all at once." },
            { icon: "⚡", title: "Live Status Sync", desc: "Admin status changes reflect immediately in the employee's ticket history." },
          ].map(f => (
            <div key={f.title} className="fcard" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18, padding: "1.75rem" }}>
              <div style={{ fontSize: 30, marginBottom: 14 }}>{f.icon}</div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 16, marginBottom: 8 }}>{f.title}</div>
...
function AuthPage({ mode, onNav, users, setUsers, admins, setAdmins, onRegistered, dark, setDark }) {
  ...
  const handleSubmit = () => {
    ...
    if (isLogin) {
      ...
      const match = all.find(u => u.email === form.email && u.password === form.password);
      ...
      if (tab === "superadmin") {
        ...
        const u = { id: generateId("SA"), name: form.name, email: form.email, password: form.password, department: "ALL", role: "superadmin" };
        setAdmins(p => [...p, u]); onRegistered({ type: "registered" });
      } else if (tab === "admin") {
        if (form.secret !== ADMIN_SECRET) return setError("Invalid admin secret code.");
        if (admins.find(a => a.email === form.email)) return setError("Email already registered.");
        const u = { id: generateId("A"), name: form.name, email: form.email, password: form.password, department: form.department, role: "admin" };
        setAdmins(p => [...p, u]); onRegistered({ type: "registered" });
      } else {
        if (users.find(u => u.email === form.email)) return setError("Email already registered.");
        const u = { id: generateId("U"), name: form.name, email: form.email, password: form.password, role: "user" };
        setUsers(p => [...p, u]); onRegistered({ type: "registered" });
      }
    }
  };
  ...
  const inp = { width: "100%", padding: "12px 14px", borderRadius: 11, border: `1.5px solid ${t.border}`, fontSize: 14, fontFamily: "inherit", outline: "none", background: t.inputBg, color: t.text, transition: "border-color 0.15s" };
...
function EmployeePortal({ user, tickets, setTickets, onLogout, dark, setDark }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!result.valid) {
        setModal({ type: "error", title: "Invalid: Resubmit Ticket", body: `Your request could not be classified. ${result.reason} Please describe a work-related issue clearly and try again.` });
      } else {
        const ticket = { id: generateId("TKT"), userId: user.id, userName: user.name, department: result.department, description, status: "Open", date: new Date().toISOString().split("T")[0] };
        setTickets(p => [ticket, ...p]);
        setDescription("");
        setModal({ type: "success", title: "Ticket Submitted! ✓", body: `Your request has been automatically routed to the ${result.department} department. Track its progress in the Ticket History tab.`, action: () => setActiveTab("history") });
      }
    } catch {
      setModal({ type: "error", title: "Connection Error", body: "Could not reach the AI classifier. Please check your connection and try again." });
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, fontFamily: "'DM Sans',sans-serif", color: t.text }}>
      ...
      <div style={{ maxWidth: 780, margin: "2.5rem auto", padding: "0 1.5rem" }}>
        ...
        {/* SUBMIT TAB — no department squares */}
        {activeTab === "submit" && (
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            <div style={{ background: t.surface, borderRadius: 18, border: `1px solid ${t.border}`, padding: "2.5rem", boxShadow: dark ? "0 2px 20px rgba(0,0,0,0.3)" : "0 2px 12px rgba(0,0,0,0.04)" }}>
              ...
              <p style={{ fontSize: 14, color: t.muted, margin: "0 0 2rem", lineHeight: 1.6 }}>Describe your issue in plain English. Our AI will automatically route it to the correct department — no need to choose.</p>
              <textarea
                value={description}
                onChange={e => setDescription(e.target.value)}
                placeholder="e.g. My laptop won't connect to the office Wi-Fi and I can't access my emails since this morning..."
                style={{ width: "100%", minHeight: 160, padding: "14px 16px", borderRadius: 13, border: `1.5px solid ${t.border}`, fontSize: 14, fontFamily: "inherit", resize: "vertical", outline: "none", lineHeight: 1.65, color: t.text, background: t.inputBg, transition: "border-color 0.15s" }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1rem" }}>
                ...
                <button onClick={handleSubmit} disabled={loading || !description.trim()}
                  style={{ padding: "12px 32px", background: loading || !description.trim() ? "#C7D2FE" : "linear-gradient(135deg,#6366F1,#8B5CF6)", border: "none", borderRadius: 12, color: "#fff", fontSize: 15, fontWeight: 700, cursor: loading || !description.trim() ? "not-allowed" : "pointer", fontFamily: "inherit", transition: "all 0.2s" }}>
          ...
          <div style={{ animation: "fadeUp 0.3s ease" }}>
            ...
            {filtered.length === 0 ? (
              <div style={{ background: t.surface, borderRadius: 18, border: `1px solid ${t.border}`, padding: "4rem 2rem", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 14, color: t.muted, marginBottom: "1.5rem" }}>{myTickets.length === 0 ? "You haven't submitted any tickets yet." : "No tickets match this filter."}</div>
                ...
                {filtered.map(tk => {
                  ...
                  return (
                    <div key={tk.id} className="hoverlift" style={{ background: t.surface, borderRadius: 14, border: `1px solid ${t.border}`, padding: "1.25rem 1.5rem", borderLeft: `4px solid ${dc}` }}>
                      ...
                      <div style={{ fontSize: 14, color: t.text, lineHeight: 1.6 }}>{tk.description}</div>
                    </div>
                  );
                })}
              </div>
...
function AdminPortal({ user, tickets, setTickets, onLogout, dark, setDark }) {
  ...
  const stats = { total: deptTickets.length, open: deptTickets.filter(tk => tk.status === "Open").length, pending: deptTickets.filter(tk => tk.status === "Pending").length, resolved: deptTickets.filter(tk => tk.status === "Resolved").length };
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, fontFamily: "'DM Sans',sans-serif", color: t.text }}>
      ...
      <div style={{ maxWidth: 1140, margin: "2rem auto", padding: "0 1.5rem" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <p style={{ fontSize: 14, color: t.muted, margin: 0 }}>Manage and resolve your department's support tickets.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 }}>
          {statCards.map(s => (
            <div key={s.label} style={{ background: t.surface, borderRadius: 14, border: `1px solid ${t.border}`, padding: "1.25rem 1.5rem" }}>
              <div style={{ fontSize: 12, color: t.muted, fontWeight: 600, marginBottom: 6, letterSpacing: 0.3 }}>{s.label.toUpperCase()}</div>
              <div style={{ fontSize: 30, fontWeight: 800, color: t.text, fontFamily: "'Syne',sans-serif", marginBottom: 12 }}>{s.value}</div>
              <div style={{ height: 5, borderRadius: 100, background: t.subtle }}>
          ...
          <div style={{ background: t.surface, borderRadius: 16, border: `1px solid ${t.border}`, padding: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 520, overflowY: "auto" }}>
              {filtered.length === 0
                ...
                : filtered.map(tk => (
                  <div key={tk.id} className="hoverlift" style={{ background: t.subtle, borderRadius: 12, padding: "1rem 1.25rem", border: `1px solid ${t.border}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      ...
                      <select value={tk.status} onChange={e => updateStatus(tk.id, e.target.value)} style={{ padding: "5px 10px", borderRadius: 8, border: `1px solid ${t.border}`, fontSize: 12, background: t.surface, color: t.text, fontFamily: "inherit", cursor: "pointer" }}>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {yTicks.map((val, i) => {
        ...
        return (
          <g key={i}>
            <line x1={padL} y1={y} x2={padL + chartW} y2={y} stroke={dark ? "#334155" : "#E2E8F0"} strokeWidth="1" strokeDasharray="4,4" />
            <text x={padL - 6} y={y + 4} textAnchor="end" fontSize="9" fill={t.muted}>{val}</text>
          </g>
        );
      })}
      {/* Y-axis line */}
      ...
      {deptStats.map((d, i) => {
        ...
        return (
          <g key={d.dept}>
            ...
            <text x={x + barW / 2} y={y - 5} textAnchor="middle" fontSize="10" fontWeight="700" fill={d.color}>{d.total}</text>
            <text x={x + barW / 2} y={padT + chartH + 18} textAnchor="middle" fontSize="9" fontWeight="600" fill={t.muted}>{d.dept}</text>
          </g>
        );
      })}
      {/* Y-axis label */}
      <text x={10} y={padT + chartH / 2} textAnchor="middle" fontSize="9" fill={t.muted} transform={`rotate(-90, 10, ${padT + chartH / 2})`}>Tickets</text>
    </svg>
  );
}
...
function PieChart({ deptStats, dark }) {
  ...
  const donut = (cx, cy, R, rr, startAngle, endAngle) => {
    ...
    const x1i = cx + rr * Math.cos(endAngle), y1i = cy + rr * Math.sin(endAngle);
    const x2i = cx + rr * Math.cos(startAngle), y2i = cy + rr * Math.sin(startAngle);
    const large = endAngle - startAngle > Math.PI ? 1 : 0;
    return `M ${x1o} ${y1o} A ${R} ${R} 0 ${large} 1 ${x2o} ${y2o} L ${x1i} ${y1i} A ${rr} ${rr} 0 ${large} 0 ${x2i} ${y2i} Z`;
  };

  return (
    <svg viewBox="0 0 260 180" style={{ width: "100%", height: "auto", display: "block" }}>
      {slices.map((s, i) => (
        <path key={i} d={s.pct > 0 ? donut(cx, cy, r, ir, s.start, s.end) : ""} fill={s.color} opacity="0.9" />
      ))}
      ...
      {/* Legend */}
      {slices.map((s, i) => (
        <g key={i} transform={`translate(170, ${20 + i * 36})`}>
          <rect x="0" y="0" width="12" height="12" rx="3" fill={s.color} />
          <text x="17" y="10" fontSize="10" fontWeight="600" fill={dark ? "#CBD5E1" : "#374151"}>{s.dept}</text>
          <text x="17" y="22" fontSize="9" fill={dark ? "#64748B" : "#94A3B8"}>{Math.round(s.pct * 100)}% · {s.total} tickets</text>
        </g>
      ))}
    </svg>
  );
...
function LineChart({ tickets, dark }) {
  ...
  const points = dates.map((d, i) => ({
    x: padL + (i / Math.max(dates.length - 1, 1)) * chartW,
  ...
  const areaPath = points.length > 0
    ? `M ${points[0].x},${padT + chartH} ` + points.map(p => `L ${p.x},${p.y}`).join(" ") + ` L ${points[points.length - 1].x},${padT + chartH} Z`
    : "";
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
      ...
      {points.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r="4" fill="#6366F1" stroke={dark ? "#1E293B" : "#fff"} strokeWidth="2" />
          <text x={p.x} y={p.y - 9} textAnchor="middle" fontSize="9" fontWeight="700" fill="#6366F1">{p.val}</text>
          <text x={p.x} y={padT + chartH + 18} textAnchor="middle" fontSize="8" fill={t.muted}>{p.date.slice(5)}</text>
        </g>
      ))}
      <text x={10} y={padT + chartH / 2} textAnchor="middle" fontSize="9" fill={t.muted} transform={`rotate(-90, 10, ${padT + chartH / 2})`}>Tickets</text>
    </svg>
...
function SuperAdminPortal({ user, tickets, setTickets, onLogout, dark, setDark }) {
  ...
  const deptStats = DEPARTMENTS.map((d, i) => ({
    ...
    total: tickets.filter(tk => tk.department === d).length,
    open: tickets.filter(tk => tk.department === d && tk.status === "Open").length,
    pending: tickets.filter(tk => tk.department === d && tk.status === "Pending").length,
    resolved: tickets.filter(tk => tk.department === d && tk.status === "Resolved").length,
  }));

  const NAV = dark ? "#0F172A" : "#0F1F3D";
    ...
    <div style={{ minHeight: "100vh", background: dark ? "#0A0F1E" : "#0F1F3D", fontFamily: "'DM Sans',sans-serif", color: "#fff" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        <div style={{ marginBottom: "2rem" }}>
          ...
          <p style={{ fontSize: 14, color: "#475569", margin: 0 }}>Full visibility across all departments and all tickets.</p>
        </div>
  ...
  return (
    <>
      ...
      <Modal show={regModal} onClose={() => { setRegModal(false); setPage("login"); }} title="Registration Successful! 🎉" type="success" btnLabel="Go to Sign In →" dark={dark}>
        Your account has been created successfully. Please sign in with your email and password to access your dashboard.
      </Modal>
    </>
  );
}